...
const { corsMiddleware, rateLimitIngest, requireAdminAuth, bodySizeLimit } = require("./middleware");
const { parseWindow, selectGranularity, ensurePath } = require("./utils");
...
app.post("/ingest/pageview", bodySizeLimit(2048), rateLimitIngest, async (req, res) => {
  try {
    ...
    res.status(400).json({ ok: false, error: error.message || "bad_request" });
  }
});
...
app.post("/analytics/compact", async (req, res) => {
  try {
    const minutes = typeof req.body === "object" && req.body && typeof req.body.minutes === "number" ? req.body.minutes : 10;
...
exports.analyticsApi = onRequest({ cors: false }, app);
