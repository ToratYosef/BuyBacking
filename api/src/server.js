      ...
      (function () {
        ...
        function wireUnlockSection(card) {
          ...
          function updateUnlockState() {
            ...
            if (method === 'pin') {
              complete = !!(pinInput && String(pinInput.value || '').trim().length);
            } else if (method === 'password') {
              complete = !!(passwordInput && String(passwordInput.value || '').trim().length);
            } else if (method === 'pattern') {
              complete = parsePatternPath(patternInput ? patternInput.value : '').length > 0;
            }

            if (resolveBtn && resolveBtn.hasAttribute('data-requires-unlock')) {
              resolveBtn.disabled = !complete;
            }
          ...
          function updatePatternUi() {
            ...
            if (patternDisplay) {
              patternDisplay.textContent = sequence.length ? ('Pattern: ' + sequence.join(' → ')) : 'Pattern not set';
            }
          ...
          if (methodSelect) {
            methodSelect.addEventListener('change', function () {
              setMethod(String(methodSelect.value || '').trim());
            });
          }
        ...
        function collectUnlockPayload(card, reason, action) {
          ...
          var unlockSection = card ? card.querySelector('[data-unlock-section]') : null;
          if (!unlockSection) {
            return { ok: false, error: 'Unlock details are required for password lock resolution.' };
          }
          ...
          if (!method || ['pin', 'password', 'pattern'].indexOf(method) === -1) {
            return { ok: false, error: 'Please select unlock method.' };
          }
          ...
          return {
            ...
            payload: {
              ...
              unlockValue: patternPath.join('-'),
              unlockPatternPath: patternPath,
            },
          };
        }
        ...
        function setFeedback(container, message, color) {
          ...
          if (color === '#16a34a') container.classList.add('success');
          if (color === '#dc2626') container.classList.add('error');
