...
// --- UPDATED: FETCHING GOOGLE PIXEL MODELS ---
...
if (a.name.includes('a')) return 1;
if (b.name.includes('a')) return -1;
...
return name
// --- UPDATED: REPLACING 'GOOGLE' INSTEAD OF 'IPHONE' ---
.toLowerCase()
.replace('google ', '')
.replace(/\s+/g, '-')
.replace(/\+/g, '-plus')
...
cardContent = `
<a href="${deviceLink}" class="desktop-image-link" data-phone-data='${JSON.stringify(phone)}'>
...
// Get data from the desktop link's HTML content
