...
async function createNotification(type, message, relatedDocId, relatedDocType, recipientId) {
    try {
        ...
        console.log(`Notification created: ${type} for document ID: ${relatedDocId}`);
    } catch (error) {
...
app.post("/submit-order", async (req, res) => {
    try {
        ...
        // Removed createNotification for order submission here
...
app.post("/generate-label/:id", async (req, res) => {
    try {
        ...
        // Removed createNotification for label generation here
...
app.put("/orders/:id/status", async (req, res) => {
    try {
        ...
        let internalSubject;
        let internalHtmlBody;
        // let notificationMessage = ''; // Removed as we're not using generic status notifications
        ...
        switch (status) {
            case "received":
                ...
                // notificationMessage = `Device received for order #${order.id}.`; // Removed
                break;
            case "completed":
                ...
                `;
                internalNotificationPromise = sendZendeskComment(order, internalSubject, internalHtmlBody, false);
                // notificationMessage = `Order #${order.id} has been marked as completed.`; // Removed
                break;
            default:
        ...
        // Removed createNotification for generic status updates here
...
app.post("/orders/:id/return-label", async (req, res) => {
    try {
        ...
        // Removed createNotification for return label generation here
...
app.post("/accept-offer-action", async (req, res) => {
    try {
        ...
        // Removed createNotification for offer acceptance here
...
app.post("/return-phone-action", async (req, res) => {
    try {
        ...
        // Removed createNotification for offer decline here
...
// NEW: Endpoint to get notifications for a specific admin or a general 'admin'
app.get("/notifications", verifyAdmin, async (req, res) => {
    const { uid } = req.user;
    try {
        const adminSpecificNotifications = await notificationsCollection
            .where('recipientId', '==', uid)
            .where('isRead', '==', false)
            .orderBy('createdAt', 'desc')
            .get();

        const generalNotifications = await notificationsCollection
        ...
        adminSpecificNotifications.docs.forEach(doc => allNotifications.push({ id: doc.id, ...doc.data() }));
        generalNotifications.docs.forEach(doc => allNotifications.push({ id: doc.id, ...doc.data() }));
...
app.put("/notifications/:id/read", verifyAdmin, async (req, res) => {
    const { id } = req.params;
    try {
...
exports.autoAcceptOffers = functions.pubsub.schedule('every 24 hours').onRun(async (context) => {
    ...
    const updates = expiredOffers.docs.map(async doc => {
        ...
        // Removed createNotification for auto-accepted offers here
...
// New Cloud Function to save user info on sign-up
...
// ------------------------------
// NEW: CHAT NOTIFICATION CLOUD FUNCTIONS
